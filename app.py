...
# Routes - FIXED WITH PROPER ENDPOINTS
@app.route('/', methods=['GET'])
def home():
    return jsonify({
        "status": "Server is running! ✅",
        "endpoints": {
            "visit": "/visit?uid=12345678&region=IND",
            "spam": "/spam?uid=12345678&region=IND"
        },
        "message": "FreeFire API Server - No 404 Errors!"
    })
...
def not_found(error):
    return jsonify({
        ...
        "available_endpoints": [
            "GET /",
            "GET /health", 
            "GET /visit?uid=12345678&region=IND",
            "GET /spam?uid=12345678&region=IND"
        ]
    }), 404
...
if __name__ == '__main__':
    ...
    print(f"[✅] Server ready! Available at: http://localhost:{port}")
    print(f"[📋] Available endpoints:")
    print(f"    GET /")
    print(f"    GET /health") 
    print(f"    GET /visit?uid=12345678&region=IND")
    print(f"    GET /spam?uid=12345678&region=IND")
